        ...
        for (int row=0;row<_rowAmount;row++) {
            for (int column=0;column<_columnAmount;column++) {
                ...
                if ([self.delegate respondsToSelector:@selector(sudokuView:cellForRowAtIndex:)]) {
                    ...
                    if ([cell isAddTitleBtn]) {
                        [cell.titleBtn addTarget:self action:@selector(clickBtn:) forControlEvents:UIControlEventTouchUpInside];
                    }
                }
            }
        }
...
- (void)clickBtn:(UIButton *)send {
    [self cellClickUpInside:(ZCSudokuCell *)[send superview]];
}
